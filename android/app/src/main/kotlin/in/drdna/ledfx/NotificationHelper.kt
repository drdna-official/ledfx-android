...
object NotificationHelper {
    ...
    fun buildNotification(context: Context, isRecording: Boolean, isPaused: Boolean): Notification {
        val state =
                when {
                    isPaused -> "Paused"
        ...
        if (isRecording) {
            ...
            val resumePending =
                    PendingIntent.getService(context, 2, resumeIntent, PendingIntent.FLAG_IMMUTABLE)
    ...
    fun updateNotification(context: Context, isRecording: Boolean, isPaused: Boolean) {
        val nm = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        nm.notify(NOTIF_ID, buildNotification(context, isRecording, isPaused))
    }
}
...
class NotificationDismissReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        // if (intent?.action == "in.drdna.ledfx.NOTIF_DISMISSED") {
            val svcRunning = RecordingService.isRunning
